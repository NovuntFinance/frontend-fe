...
export function DepositDialog({ trigger, className }: DepositDialogProps) {
  ...
  const [depositDetails, setDepositDetails] = useState<InitiateDepositResponse | null>(null);
  ...
  const expiresLabel = useMemo(() => {
    ...
    try {
      return formatDistanceToNow(new Date(depositDetails.expiresAt), { addSuffix: true });
    } catch {
      return null;
    }
        ...
        {!depositDetails && (
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <Button type="submit" className="w-full" disabled={initiateDeposit.isPending}>
                {initiateDeposit.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                Generate Deposit Address
              </Button>

              <p className="text-xs text-muted-foreground text-center">
                You&apos;ll receive a unique USDT (TRC20) address. Send the exact amount within one hour to avoid expiration.
              </p>
            </form>
          </Form>
        )}
        ...
        {depositDetails && (
          <div className="space-y-5">
            <div className="rounded-lg border bg-muted/40 p-4">
              ...
              <p className="text-2xl font-semibold">{formatCurrency(depositDetails.amount, 'USDT')}</p>
              <p className="text-xs text-muted-foreground mt-1">
                Network: {depositDetails.network ?? 'TRC20'}
              </p>
              {expiresLabel && (
            ...
            <div className="space-y-2">
              ...
              <code className="block break-words rounded-md bg-muted px-3 py-2 text-sm">
            ...
            <div className="rounded-lg border border-amber-200/70 bg-amber-50/60 p-4 text-sm leading-relaxed dark:border-amber-500/40 dark:bg-amber-500/10">
              <ul className="space-y-2">
                <li>• Only send USDT on the TRC20 network to this address.</li>
                <li>• Deposits are credited automatically after blockchain confirmation.</li>
                <li>• Sending a different token or amount may result in loss of funds.</li>
              </ul>
            </div>
