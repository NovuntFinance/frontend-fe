...
const AdminMetricCard = ({
  ...
  const maxSparklineValue = effectiveSparkline
    ? Math.max(...effectiveSparkline) || 0
    : 0;
  const barColorClass = sparklineColor ?? 'bg-indigo-500';

  return (
    <div
      className={`h-full min-h-[160px] rounded-lg border bg-white shadow-sm dark:bg-gray-800 ${alert ? 'border-amber-300 dark:border-amber-700' : 'border-gray-200 dark:border-gray-700'} p-5`}
    >
      <div className="flex justify-between">
        ...
        <div
          className={`flex h-12 w-12 items-center justify-center rounded-md ${alert ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400'}`}
        >
          {getIcon()}
        </div>
      </div>
          ...
          <div className="flex h-full items-end space-x-1">
            {effectiveSparkline
              ...
              : Array.from({ length: 12 }).map((_, index) => (
                  <div
                    key={index}
                    className="rounded-sm bg-indigo-100 dark:bg-indigo-700/30"
                    style={{
