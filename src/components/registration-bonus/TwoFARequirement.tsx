/**
 * 2FA Requirement Component – Gold Design
 * Shows whether the user has set up Two-Factor Authentication
 */
...
import type { TwoFARequirementProps } from '@/types/registrationBonus';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';

export function TwoFARequirement({
  twoFAData,
  onComplete,
}: TwoFARequirementProps) {
  const isComplete = twoFAData.isCompleted;
        ...
        <div className="mb-3 flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-novunt-gold-500/20 border-novunt-gold-500/30 group-hover:bg-novunt-gold-500/30 rounded-lg border p-2.5 transition-colors">
              <ShieldCheck className="text-novunt-gold-600 dark:text-novunt-gold-500 h-5 w-5" />
            </div>
            <div>
              <h3 className="text-foreground mb-0.5 text-sm font-bold">
                Setup 2FA
