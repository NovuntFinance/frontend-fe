...
export const ToastMessages = {
    ...
    // Wallet operations
    depositInitiated: () =>
        toast.loading('Processing deposit...', { description: 'Please complete payment in the popup window' }),

    depositSuccess: (amount: number) =>
        toast.success('Deposit successful!', { description: `$${amount.toFixed(2)} has been added to your account` }),

    withdrawalInitiated: () =>
        toast.loading('Processing withdrawal...', { description: 'This may take a few minutes' }),

    withdrawalSuccess: (amount: number) =>
        toast.success('Withdrawal requested!', { description: `$${amount.toFixed(2)} will be sent to your account` }),

    // Staking  
    stakeCreated: (amount: number) =>
        toast.success('Stake created!', { description: `Successfully staked $${amount.toFixed(2)}` }),
    ...
    validationError: (field: string) =>
        toast.error('Validation error', { description: `Please check the ${field} field` }),
    ...
    deleteSuccess: () =>
        toast.success('Deleted!', { description: 'Item has been deleted successfully' }),
};
