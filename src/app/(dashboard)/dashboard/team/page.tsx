...
import { useReferralStats, useReferralInfo, useReferralTree, useTeamInfo } from '@/lib/queries';
...
export default function TeamPage() {
  ...
  const { data: referralTree, isLoading: treeLoading } = useReferralTree(maxLevels);
  ...
  const treeByLevel = referralTree?.tree?.reduce((acc, entry) => {
    if (!acc[entry.level]) {
      acc[entry.level] = [];
    }
  ...
  // Filter downlines
  const filteredDownlines = teamInfo?.directDownlines?.filter((downline) => {
    const matchesSearch = 
      !searchQuery ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        >
          <Card className="bg-gradient-to-br from-purple-900/20 via-purple-800/10 to-transparent border-purple-500/20">
            <CardContent className="p-6">
              ...
              <div className="space-y-2">
                <p className="text-sm font-medium text-purple-400">Total Referrals</p>
                ...
                <Progress 
                  value={referralStats?.totalReferrals ? Math.min(100, (referralStats.activeReferrals || 0) / referralStats.totalReferrals * 100) : 0} 
        ...
        >
          <Card className="bg-gradient-to-br from-green-900/20 via-green-800/10 to-transparent border-green-500/20">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="space-y-2">
                <p className="text-sm font-medium text-green-400">Total Earned</p>
                <p className="text-3xl font-bold text-white">
                ...
                <Progress 
                  value={referralStats?.totalEarned ? Math.min(100, (referralStats.totalEarned / 10000) * 100) : 0} 
        ...
        >
          <Card className="bg-gradient-to-br from-blue-900/20 via-blue-800/10 to-transparent border-blue-500/20">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="space-y-2">
                <p className="text-sm font-medium text-blue-400">Team Members</p>
                <p className="text-3xl font-bold text-white">
                ...
                <Progress 
                  value={teamInfo?.teamStats?.totalTeamMembers ? Math.min(100, (teamInfo.teamStats.totalDirectDownlines || 0) / teamInfo.teamStats.totalTeamMembers * 100) : 0} 
      ...
      {activeTab === 'referrals' && (
        <div className="space-y-6">
          ...
          >
            <Card className="bg-gradient-to-br from-purple-900/20 via-blue-900/10 to-transparent border-purple-500/20">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                {/* Referral Link */}
                <div>
                  <label className="text-sm font-medium mb-2 block text-muted-foreground">Referral Link</label>
                  {infoLoading ? (
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <Button variant="outline" onClick={copyLink} className="border-purple-500/20">
                        {copiedLink ? (
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span>Referral link not available. Please refresh the page.</span>
                    </div>
                  )}
                  {copiedLink && (
                ...
                {/* Referral Code */}
                <div>
                  <label className="text-sm font-medium mb-2 block text-muted-foreground">Referral Code</label>
                  {infoLoading ? (
                    <Skeleton className="h-10 w-full" />
                  ) : referralCode ? (
                    <div className="flex gap-2">
                      <Input
                        value={referralCode}
                        readOnly
                        className="font-mono text-lg font-bold tracking-wider bg-background/50 border-purple-500/20"
                      />
                      <Button variant="outline" onClick={copyCode} className="border-purple-500/20">
                        {copiedCode ? (
                          <CheckCircle className="h-4 w-4 text-green-500" />
                        ) : (
                          <Copy className="h-4 w-4" />
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <AlertCircle className="h-4 w-4" />
                      <span>Referral code not available. Please refresh the page.</span>
                    </div>
                  )}
                  {copiedCode && (
                    <p className="text-xs text-green-500 mt-2">Code copied to clipboard!</p>
                  )}
                </div>
              </CardContent>
            </Card>
          ...
          >
            <Card className="bg-gradient-to-br from-slate-900/20 via-slate-800/10 to-transparent border-slate-500/20">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle>Referral Tree</CardTitle>
                    <CardDescription>
                      Your referral network structure (levels 1-5 receive bonuses)
                    </CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="px-3 py-1.5 text-sm border rounded-lg bg-background/50 border-slate-500/20"
                    >
                  ...
                  <div className="space-y-3">
                        ...
                        >
                          ...
                          <button
                            ...
                            className={`w-full p-4 flex items-center justify-between transition-all ${
                              hasEntries ? 'cursor-pointer hover:opacity-90' : 'cursor-default opacity-60'
                            }`}
                            style={hasEntries ? {
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div 
                                className={`p-3 rounded-xl border-2 ${colors.border}`}
                                style={{
                              ...
                              >
                                {hasEntries && isExpanded ? (
                                  <ChevronDown className={`h-6 w-6 ${colors.iconColor}`} />
                                ) : hasEntries ? (
                                  <ChevronRight className={`h-6 w-6 ${colors.iconColor}`} />
                                ) : (
                              ...
                              <div className="text-left">
                                <div className="flex items-center gap-2">
                                  <Badge 
                                    className={`${colors.badgeBg} ${colors.badgeText} text-sm font-bold px-3 py-1`}
                                    style={{
                                  ...
                                  <span className={`text-sm font-semibold ${colors.labelColor}`}>
                                    {REFERRAL_COMMISSION_RATES[`level${level}` as keyof typeof REFERRAL_COMMISSION_RATES]}% commission
                                  </span>
                                </div>
                                <p className="text-xs text-muted-foreground mt-1">
                                  {entries.length} referral{entries.length !== 1 ? 's' : ''}
                                </p>
                              </div>
                            </div>
                            {hasEntries && (
                              <div 
                                className={`text-sm font-bold ${colors.labelColor} px-3 py-1 rounded-lg`}
                                style={{
                                  backgroundColor: `${colors.hex}30`,
                                }}
                              >
                                  ...
                                  <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                                    {entries.map((entry, index) => (
                                      <motion.div
                                        ...
                                        className={`p-3 rounded-lg border-2 hover:opacity-80 transition-all shadow-md`}
                                        style={{
                                      ...
                                      >
                                        <div className="flex items-center gap-2 mb-2">
                                          ...
                                          <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium truncate text-white">{entry.username}</p>
                                            <p className="text-xs text-muted-foreground truncate">{entry.email}</p>
                                          </div>
                                        </div>
                                        <div className="flex items-center justify-between text-xs">
                                          <span className="text-muted-foreground">
                                            {formatDate(new Date(entry.joinedAt))}
                                          </span>
                      ...
                      .map(([level, entries]) => {
                        ...
                        return (
                          <motion.div
                            ...
                            className="rounded-lg border-2 border-slate-500/20 bg-gradient-to-br from-slate-900/20 via-slate-800/10 to-transparent overflow-hidden"
                          >
                            <button
                              onClick={() => hasEntries && toggleLevel(levelNum)}
                              disabled={!hasEntries}
                              className={`w-full p-4 flex items-center justify-between transition-colors ${
                                hasEntries ? 'hover:bg-background/20 cursor-pointer' : 'cursor-default opacity-60'
                              }`}
                            >
                              <div className="flex items-center gap-3">
                                    ...
                                    <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                                      {entries.map((entry, index) => (
                                        <motion.div
                                          ...
                                          animate={{ opacity: 1, scale: 1 }}
                                          transition={{ delay: index * 0.05 }}
                                          className="p-3 rounded-lg bg-background/50 border border-slate-500/20 hover:bg-background/70 transition-colors"
                                        >
                                          <div className="flex items-center gap-2 mb-2">
                                            <Avatar className="h-8 w-8">
                                              <AvatarFallback className="text-xs bg-gradient-to-br from-slate-500/20 to-slate-500/40 text-slate-400">
                                                {entry.username.charAt(0).toUpperCase()}
                                              </AvatarFallback>
                                            </Avatar>
                                            <div className="flex-1 min-w-0">
                    ...
                    {referralTree.note && (
                      <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                        <p className="text-sm text-blue-600 dark:text-blue-400">{referralTree.note}</p>
                      </div>
                    )}
                  </div>
                ) : (
      ...
      {activeTab === 'team' && (
        <div className="space-y-6">
          {/* Rank Distribution */}
          {teamInfo?.teamStats?.rankDistribution && Object.keys(teamInfo.teamStats.rankDistribution).length > 0 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.4 }}
            >
              <Card className="bg-gradient-to-br from-slate-900/20 via-slate-800/10 to-transparent border-slate-500/20">
                ...
                <CardContent>
                  <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                    {Object.entries(teamInfo.teamStats.rankDistribution).map(([rank, count], index) => (
                      <motion.div
                        key={rank}
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: 0.5 + index * 0.1 }}
                        className="flex items-center justify-between p-3 rounded-lg bg-background/50 border border-slate-500/20"
                      >
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="relative flex-1">
                    ...
                    <Input
                      ...
                      className="pl-10 bg-background/50 border-slate-500/20"
                    />
                  </div>
                  ...
                  <div className="space-y-3">
                    {filteredDownlines.map((downline, index) => (
                      <motion.div
                        ...
                        className="flex items-center justify-between p-4 rounded-lg bg-background/50 border border-slate-500/20 hover:bg-background/70 transition-colors"
                      >
                        <div className="flex items-center gap-4 flex-1 min-w-0">
                          <Avatar className="h-12 w-12">
                            <AvatarFallback className="bg-gradient-to-br from-blue-500/20 to-blue-500/40 text-blue-400 font-bold">
                              {downline.username.charAt(0).toUpperCase()}
                            </AvatarFallback>
                          </Avatar>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <p className="font-semibold truncate">{downline.username}</p>
                              <Badge variant="outline" className="text-xs">
                                {downline.rank}
                              </Badge>
                              {downline.isActive && (
                            ...
                            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                              <span>
                                Personal: <span className="font-medium text-foreground">{formatCurrency(downline.personalStake)}</span>
                              </span>
                              <span>
                                Team: <span className="font-medium text-foreground">{formatCurrency(downline.teamStake)}</span>
                              </span>
                              <span>
                                Joined: <span className="font-medium text-foreground">{formatDate(new Date(downline.createdAt))}</span>
                              </span>
                            </div>
                          </div>
                        </div>
                        {downline.directDownlines && downline.directDownlines.length > 0 && (
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <span>{downline.directDownlines.length} sub-teams</span>
