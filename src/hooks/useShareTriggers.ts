...
export function useShareTriggers() {
  useEffect(() => {
    ...
    const handleProfitMilestone = (event: CustomEvent) => {
      ...
      const milestone = PROFIT_MILESTONES.filter(
        (m) => totalEarnings >= m
      ).sort((a, b) => b - a)[0];
    ...
    window.addEventListener(
      'profit:milestone',
      handleProfitMilestone as EventListener
    );
    window.addEventListener(
      'withdrawal:success',
      handleWithdrawalSuccess as EventListener
    );
    window.addEventListener(
      'stake:completed',
      handleStakeCompleted as EventListener
    );
    window.addEventListener(
      'rank:achieved',
      handleRankAchieved as EventListener
    );
    window.addEventListener(
      'bonus:completed',
